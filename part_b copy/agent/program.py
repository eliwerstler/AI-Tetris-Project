...
class Agent:
    def __init__(self, color: PlayerColor, **referee: dict):
        ...
        self._state = [
            [None for _ in range(BOARD_N)]
            for _ in range(BOARD_N)
        ]
        self._opponent_color = PlayerColor.RED if self._color == PlayerColor.BLUE else PlayerColor.BLUE
        print(f"Initializing Agent as {self._color}")

    # Function to determine the action to take
    def action(self, **referee: dict) -> Action:
        start_time = time.time()
        time_limit = 5.0  # Set a time limit for the search
    ...
    # Function to update the internal state based on the action taken
    ...
    def rollout(self, state):
        ...
        while not self.is_terminal(current_state):
            ...
            current_color = self._opponent_color if current_color == self._color else self._color
    ...
    def best_action(self, root: Node):
        ...
        if best_child and self.is_legal_move(root.state, best_child.action, self._color):
    ...
    def is_terminal(self, state):
        return not any(self.is_legal_shape(state, r, c, piece_type, self._color)
                       for r in range(BOARD_N) for c in range(BOARD_N) for piece_type in PieceType)

    # Function to evaluate a state (used in the rollout)
    def evaluate(self, state):
        my_pieces = sum(cell == self._color for row in state for cell in row)
        opponent_pieces = sum(cell == self._opponent_color for row in state for cell in row)
    ...
    def is_legal_shape(self, state, r: int, c: int, piece_type: PieceType, color: PlayerColor) -> bool:
        try:
            piece = create_piece(piece_type, Coord(r, c))
            coords = piece.coords
            if all(0 <= coord.r < BOARD_N and 0 <= coord.c < BOARD_N and state[coord.r][coord.c] is None for coord in coords):
